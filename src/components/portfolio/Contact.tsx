      ...
      <div className="container">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          >
            ...
            <h2 className="font-display text-4xl sm:text-5xl lg:text-7xl font-bold leading-[0.95] mb-8">
            ...
            <p className="text-base sm:text-lg text-muted-foreground max-w-lg leading-relaxed mb-10">
              Disponible pour des missions en marketing digital, création de contenu, graphisme ou montage vidéo. Envoyez-moi un message via le formulaire ou utilisez mes coordonnées directes.
            </p>
            ...
            <div className="space-y-3 mb-10">
              {links.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className="group flex items-center gap-4 sm:gap-6 p-4 sm:p-5 rounded-2xl bg-card-gradient border border-border hover:border-primary/40 transition-all duration-300"
                >
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center group-hover:bg-primary group-hover:text-primary-foreground transition-all shrink-0">
                    <link.icon className="w-5 h-5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs uppercase tracking-widest text-muted-foreground mb-1">{link.label}</div>
                    <div className="font-display text-sm sm:text-base font-medium truncate">{link.value}</div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-4 sm:p-5 rounded-2xl border border-border bg-secondary/30">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {["Français", "Anglais", "Éwé"].map((l) => (
                    <span key={l} className="px-2.5 py-1 rounded-full bg-background border border-border text-xs">{l}</span>
                  ))}
                </div>
              </div>
              <div className="p-4 sm:p-5 rounded-2xl border border-border bg-secondary/30">
          ...
          >
            <div>
              <label className="text-xs uppercase tracking-widest text-primary mb-2 block">Votre nom</label>
              <input
                type="text"
                value={form.name}
            ...
            <div>
              <label className="text-xs uppercase tracking-widest text-primary mb-2 block">Votre email</label>
              <input
                type="email"
                value={form.email}
            ...
            <div>
              <label className="text-xs uppercase tracking-widest text-primary mb-2 block">Votre message</label>
              <textarea
                value={form.message}
            ...
            <button
              type="submit"
              disabled={loading}
              className="group w-full inline-flex items-center justify-center gap-2 px-6 py-4 rounded-full bg-primary text-primary-foreground font-medium hover:shadow-glow transition-all duration-500 disabled:opacity-60"
            >
